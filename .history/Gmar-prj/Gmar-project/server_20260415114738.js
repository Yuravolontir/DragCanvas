...
function calculateNextRunDate(frequency, scheduleTime, scheduleDay) {
 ...
 app.get('/api/users', async (req, res) => {
  ...
  app.get('/api/users/:id', async (req, res) => {
    try {
       const response = await fetch(`http://localhost:5172/api/users/${req.params.id}`);
  ...
  app.post('/api/login', async (req, res) => {
    try {
      ...
      // Use stored procedure for login
       const response = await fetch(`http://localhost:5172/api/login/${email}/${password}/${req.ip || 'unknown'}`);
       const user = await response.json();



      if (user.resultCode !== 1 || !user.userID) {
