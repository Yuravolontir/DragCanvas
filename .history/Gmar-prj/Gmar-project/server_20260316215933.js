...
app.delete('/api/delete-user', async (req, res) => {
  ...
  request.execute('dbo.SP_UpdateUserRole');
      ...
      try {
        ...
        console.log('projectSizeKB:', projectSizeKB);
  ...
  0);
        ...
        const request = pool.request()
          ...
          .input('ProjectDescription', sql.NVarChar(500),
  projectDescription || null)
          .input('ComponentCount', sql.Int, componentCount || 0)
          .input('ProjectSizeKB', sql.Decimal(10,2), projectSizeKB
  || 0)
          .input('ProjectData', sql.NVarChar(sql.MAX), projectData
  || null)
        ...
        console.log('result.output.ResultCode:',
  result.output?.ResultCode);
        console.log('result.output.ResultProjectID:',
  result.output?.ResultProjectID);
  ...
  resultProjectId);
          res.json({ projectId: resultProjectId, message: 'Project
  saved successfully' });
        } else if (resultCode === 2) {
          console.log('❌ Maximum projects limit reached');
          res.status(400).json({ error: 'Maximum projects limit
  reached' });
        } else {
          console.log('❌ Failed to save project, ResultCode:',
  resultCode);
