...
export default function AdminPanel() {
 ...
 <InputGroup className="mb-3 me-3">
  ...
  bg="secondary">Inactive</Badge>}
                   {user.IsSuperAdmin ? (
      <Badge bg="danger" className="ms-2">SuperAdmin</Badge>
    ) : user.IsAdmin ? (
      <Badge bg="danger" className="ms-2">Admin</Badge>
    ) : null}
                  </td>
                            <td>
              {/* Delete button - not for admins, superadmins, or yourself
            */}
              {!user.IsAdmin && !user.IsSuperAdmin && user.User_ID !==
            currentUser?.User_ID && (
            ...
            handleDeleteClick(user)}>
              ...
              {/* Deactivate button - not for superadmins or yourself */}
              {user.IsActive && !user.IsSuperAdmin && !user.IsAdmin &&
            user.User_ID !== currentUser?.User_ID && (
                <Button variant="warning" size="sm" onClick={() =>
            handleUpdateStatusClick(user, false)}>
                  Deactivate
                </Button>
              )}

              {/* Activate button - not for superadmins (they should always
            be active) */}
              {!user.IsActive && !user.IsSuperAdmin && (
                <Button variant="success" size="sm" onClick={() =>
            handleUpdateStatusClick(user, true)}>
                  Activate
                </Button>
              )}
              <p></p>

              {/* Reset Password - not for superadmins or yourself */}
            ...
            handleResetPasswordClick(user)}>
              ...
              {/* Make Admin - only superadmin, not for superadmins or
            yourself */}
              {currentUser?.IsSuperAdmin && !user.IsSuperAdmin &&
            !user.IsAdmin && user.User_ID !== currentUser?.User_ID && (
            ...
            handleRoleChangeClick(user, true)}>
              ...
              {/* Remove Admin - only superadmin, only for regular admins
            (not superadmins) */}
              {currentUser?.IsSuperAdmin && !user.IsSuperAdmin &&
            user.IsAdmin && user.User_ID !== currentUser?.User_ID && (
