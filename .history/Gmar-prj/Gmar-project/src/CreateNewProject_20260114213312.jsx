  ...
  export default function CreateNewProject() {
    ...
    return (
      <>
        <NavBar />
        <div style={{ display: 'grid', gridTemplateColumns: '200px 1fr 200px', height: 'calc(100vh - 60px)' }}>
          <DndContext onDragEnd={handleDragEnd}>
            <div style={{ border: '1px solid #ccc', padding: '10px', height: '100%', display: 'flex', flexDirection: 'column', gap: '10px' }}>
              <h3>Add Elements</h3>
            ...
            <div style={{ border: '2px dashed #ccc', position: 'relative', height: '100%', backgroundColor: '#f5f5f5', overflow: 'hidden' }}>
              {elements.map(el => (
          ...
          <div style={{ border: '1px solid #ccc', padding: '10px', height: '100%', display: 'flex', flexDirection: 'column', gap: '10px' }}>
            <h3>Actions</h3>
            <button onClick={saveLayout} style={{ padding: '10px' }}>Save</button>
            <button onClick={clearLayout} style={{ padding: '10px' }}>Clear</button>
          </div>
        </div>
      </>
    );
