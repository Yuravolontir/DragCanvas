...
`.trim();
    ...
    // Read as text first (so we can debug even if not JSON)
    ...
    const raw = data?.choices?.?.message?.content; // <-- correct optional chaining[1]
    if (!raw) {
      return res.status(500).json({ error: 'No choices.message.content', body: data });
    }
...
function safeParseAIJson(rawText) {
  ...
  // strip markdown link format if leaked: [url](url) -> url
