...
- Use ONLY the URLs listed below as plain strings (no [text](url) markdown).
...
`.trim();
    ...
    const response = await fetch('https://api.perplexity.ai/chat/completions', {
      ...
      headers: {
        ...
        Authorization: `Bearer ${process.env.PPLX_API_KEY}`, // <-- PUT KEY IN ENV!
      },
      body: JSON.stringify(requestData),
    });

    const data = await response.json();
    const raw = data?.choices?.?.message?.content;

    if (!raw) return res.status(500).json({ error: 'Invalid AI response' });

    // --- Robust JSON cleanup + parse on the backend ---
    const parsed = safeParseAIJson(raw);

    if (!parsed?.sections || !Array.isArray(parsed.sections)) {
      return res.status(400).json({
        error: 'AI did not return sections[]',
        rawPreview: raw.slice(0, 800),
      });
    }

    // Return actual object; frontend will not JSON.parse strings.
    res.json(parsed);
...
// --- Helper: tries hard to extract + parse valid JSON ---
function safeParseAIJson(rawText) {
  ...
  // keep only first JSON object boundaries
  ...
  // If AI accidentally returned markdown links in strings, strip to URL:
