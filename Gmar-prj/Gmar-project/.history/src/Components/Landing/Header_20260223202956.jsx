    ...
    try {
      ...
      if (!userResponse.ok) {
        if (userResponse.status === 401) {
          throw new Error('Invalid token. Please check your Netlify Personal AccessToken.');
        }
      ...
      // Deploy using the deploy endpoint with files
      const deployResponse = await
  fetch(`https://api.netlify.com/api/v1/sites/${site.id}/deploys`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      alert(`✅ Deployed!\n\nView at: https://${site.name}.netlify.app`);
