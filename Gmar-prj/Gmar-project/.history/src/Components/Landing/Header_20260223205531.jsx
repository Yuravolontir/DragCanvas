...
async function deployToNetlify(html, token) {
  try {
    ...
    let deploy = await deployResponse.json();
    console.log('Deploy started:', deploy.id);

    // 4️⃣ WAIT until deploy is ready
    ...
    const liveUrl = deploy.ssl_url || deploy.deploy_ssl_url;

    if (!liveUrl) {
      throw new Error('No live URL returned.');
    }
