...
`;
 ...
 async function deployToNetlify(html, token) {
  try {
    ...
    // Check if we already created a site before
    ...
    if (!siteId) {
      ...
      if (!siteResponse.ok) {
        throw new Error('Failed to fetch existing site.');
      }
    ...
    const uploadResponse = await fetch(
      `https://api.netlify.com/api/v1/sites/${siteId}/files/index.html`,
      {
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${token}`,
