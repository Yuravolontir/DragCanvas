...
`;
 ...
 async function deployToNetlify(html, token) {
  try {
    ...
    // 3️⃣ Create deploy with file hash
    const encoder = new TextEncoder();
    const contentBuffer = encoder.encode(html);

    const digestBuffer = await crypto.subtle.digest('SHA-1', contentBuffer);
    ...
    // 4️⃣ Upload file to required upload URL
    if (deploy.required && deploy.required.length > 0) {
      const uploadUrl = `https://api.netlify.com/api/v1/deploys/${deploy.id}/files/index.html`;
