...
`;
 ...
 async function deployToTiiny(html) {
    try {
      const siteName = `my-site-${Date.now()}`;

      const response = await fetch('https://api.tiiny.host/upload', {
        method: 'POST',
        headers: {
      ...
      const siteUrl = result.url || `https://${siteName}.tiiny.host`;
