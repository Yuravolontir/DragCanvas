...
`;
  ...
  async function deployToSurge(html) {
    try {
      ...
      // Deploy to surge (no auth needed for basic use)
      const response = await fetch('https://surge.sh/upload', {
        method: 'POST',
        body: formData
      });
      ...
      if (response.ok) {
        ...
        alert(`Deployed! View at: ${result.url}`);
      } else {
        throw new Error('Surge deployment failed');
      }
    } catch (error) {
