...
export default function AIAssistant() {
    ...
    for (const section of sections) {
      ...
      if (Array.isArray(section.children)) {
        for (const child of section.children) {
          ...
          const resolvedName =
            child.type ? child.type.charAt(0).toUpperCase() + child.type.slice(1) : 'Text';
  ...
  const generateWebsite = async () => {
    ...
    try {
      ...
      // If this fails later, it means Craft expects its own serialized format,
      // but for now we keep your current approach.
